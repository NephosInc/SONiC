...
def init_logging(logging_level=logging.WARNING):
    global g_sfp_logger
...
def init_phy_port_media_mdio_config(phy_port_info, sfp_info):
    ...
    for port in all_user_port_value:
        ...
        if port_lanes_cnt == 1:
            ...
            media_value = media_abbr[1] + str(port_lanes_cnt)
...
def get_phy_port_admin_status(user_port_list):
    ...
    for r in result:
        ...
        nps_port = user_port_list[user_port][KEY_PHY_PORT_INFO_V_USER_PORT_V_NPS_PORT]
        port_admin_status[user_port] = (status,nps_port)
    ...
    for user_port, (admin, (port_unit, port_index))  in port_admin_status.items():
        if str(admin).lower() == 'up'.lower():
            ...
            if cmd_out[1] != '':
                ...
                g_sfp_logger.error("cmd={} type cmd_out {}, cmd_out:{}\n".format(cmd, type(cmd_out), cmd_out))
                ret = -1
    npx_admin_up_user_ports(port_admin_status) 
...
def npx_admin_up_user_ports(popen_obj, port_admin_status):
    exec_cmd = []

    for user_port, (admin, (port_unit, port_index)) in port_admin_status.items():
        if str(admin).lower() == 'up'.lower():
            k = KEY_PHY_PORT_INFO_V_USER_PORT_V_NPS_PORT
            cmd_str = 'npx_diag port set property unit={} portlist={} admin=enable'.format(
...
def get_port_running_config_exec_cmd(popen_obj, pre_emphasis_config, user_port_list):
    ...
    for port_name in user_port_list:
        ...
        cmd_str = 'npx_diag port show property unit={0} portlist={1}\n'.format(
            unit, port)
        popen_obj.stdin.write(cmd_str)
        exec_cmd.append(cmd_str)
        time.sleep(0.005)
...
def set_port_running_config(media_changed,
    ...
    if pre_emphasis_changed:
        g_sfp_logger.critical("worker {} change port {} pre-emphasis from {} to {} succeed\n".format(
            worker_id, user_port_list, running_pre_emphasis, pre_emphasis_config))
...
def sfp_monitor_by_group(port_list_dict, mask_ports, msg_queue):
    ...
    worker_id = threading.current_thread().name
    all_changed_cmd = []
    for phy_port_index in sorted(map(int, port_list_dict.keys())):
        phy_port = str(phy_port_index)
